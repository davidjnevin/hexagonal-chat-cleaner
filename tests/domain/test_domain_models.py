...
def test_if_chat_dto_can_be_created_with_more_than_2000_characters():
    with pytest.raises(marshmallow.exceptions.ValidationError):
        schema_ = ChatCreateDTO()
        dirty_text = "a" * 2001
        result = schema_.load({"chat": dirty_text})
        assert result == {"chat": "a" * 2001}
...
def test_if_cleaning_is_created_with_factory_and_wrong_type():
    schema_ = CleaningCreateDTO()
    with pytest.raises(marshmallow.exceptions.ValidationError):
        result = schema_.load({"chat": 1, "cleaned_chat": "test"})
        model_ = cleaning_factory(**result)
        assert model_.chat == "\n19:00:54 David said:\ntest"
        assert model_.cleaned_chat == "test"
    with pytest.raises(marshmallow.exceptions.ValidationError):
        result = schema_.load({"chat": "text", "cleaned_chat": 1})
        model_ = cleaning_factory(**result)
        assert model_.chat == "\n19:00:54 David said:\ntest"
        assert model_.cleaned_chat == "test"
...
def test_if_cleaning_is_created_with_empty_strings():
    schema_ = CleaningCreateDTO()
    with pytest.raises(marshmallow.exceptions.ValidationError):
        result = schema_.load({"chat": "", "cleaned_chat": "test"})
        model_ = cleaning_factory(**result)
        assert model_.chat == ""
        assert model_.cleaned_chat == "test"
    with pytest.raises(marshmallow.exceptions.ValidationError):
...
def test_if_cleaning_is_created_with_more_than_2000_characters():
    ...
    with pytest.raises(marshmallow.exceptions.ValidationError):
        dirty_text = "a" * 2001
        result = schema_.load({"chat": dirty_text, "cleaned_chat": "test"})
        model_ = cleaning_factory(**result)
        assert model_.chat == dirty_text
        assert model_.cleaned_chat == "test"
    with pytest.raises(marshmallow.exceptions.ValidationError):
        dirty_text = "a" * 2001
        result = schema_.load(
